...
MPCr0 = MPCdc;
MPCr0.branch(:,BR_R) = 0;
MPCr0.branch(:,BR_B) = 0*MPCr0.branch(:,BR_B);
%MPCr0.bus(:,QD) = MPCr0.bus(:,PD)/0.98*sqrt(1-0.98^2);
MPCr0.bus(:,QD) = 0;
MPCr0 = runpf(MPCr0);
...
for i = 1:nsteps
    ...
    %MPCr0.bus(:,QD) = -100*dQ_dVrms*(1-MPCr0.bus(:,VM));
    MPCr0.bus(:,QD) = median([-abs(MPCr0.bus(:,PD))/0.9*sqrt(1-0.9^2),abs(MPCr0.bus(:,PD))/0.9*sqrt(1-0.9^2), MPCr0.bus(:,QD)-100*dQ_dVrms*(1-MPCr0.bus(:,VM))], 2);
    MPCr0 = runpf(MPCr0);
